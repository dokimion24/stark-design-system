    ...
    function handleKeyDown(e: KeyboardEvent) {
      ...
      if (e.code === 'Enter' || e.code === 'Space') {
        ...
        if (triggerIsFocused) {
          ...
          const targetOption = (Array.from(optionListElement.children) as HTMLLIElement[]).find(
            (optionElement) => optionElement.contains(focusedElement),
          );
          if (targetOption) {
            e.preventDefault();
        ...
        const focusableOptionElements = optionElements.filter(
...
export const useOptionListPosition = ({
  ...
  const changeOptionListPosition = useCallback(() => {
    ...
    if (optionListElement && triggerElement) {
      ...
      optionListElement.style.top = `${windowScrollY + triggerBoundingRect.bottom + 2}px`;
