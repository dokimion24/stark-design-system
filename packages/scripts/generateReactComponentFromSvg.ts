...
const SVG_DIR = '../stark-icons/src/svg';
const COMPONENT_DIR = '../stark-icons/src/component';
...
const generateExportFile = async (components: string[]) => {
  const EXPORT_FILE_PATH = '../stark-icons/src/component/index.ts';
  const exportFileContent = components
    .map((component) => `export { default as ${component} } from "./${component}.tsx";`)
    .join('\n');
